...
function GetCourier()
{
	...
	const tokenId = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJMb2dpbklERW5jcnlwdCI6IkdJb2RoSmpZblVKTkN4MmMwVWFZeGc9PSIsIm5iZiI6MTcxMDIzODU4MSwiZXhwIjoxNzEwMjM5MTgxLCJpYXQiOjE3MTAyMzg1ODF9.gcf6uSXgZmyLRYJ1t_FXbx0Aja7sG6jkLZr3nmsszRo';
		        ...
		        <div className="d-flex justify-content-between"  style={{ marginTop: '1rem', }}>
					...
					<div>
						...
						<button type="button" className="btn btn-light btn-outline-danger" 
							...
							onClick={() => { 
								setPgNo(totalResults === 0 ? 1 : (pgNo - 1) === 0 ? 1 : (pgNo - 1) === Math.ceil(totalResults/pgSize) - 1 ? Math.ceil(totalResults/pgSize) - 2  : pgNo - 1);
							}
						}> 
							{totalResults === 0 ? 1 : (pgNo - 1) === 0 ? 1 : (pgNo - 1) === Math.ceil(totalResults/pgSize) - 1 ? Math.ceil(totalResults/pgSize) - 2  : pgNo - 1} 
						</button>

						<button type="button" className="btn btn-light btn-outline-danger" 
							disabled={(totalResults === 0) || totalResults < pgSize || (pgNo === 1 ? false : (pgNo) === Math.ceil(totalResults/pgSize) ? false : true)}
							onClick={() => { 
								setPgNo(pgNo >= Math.ceil(totalResults/pgSize) ?  pgNo-1 : pgNo+1);
							}
						}>
							{totalResults === 0 ? 2 : pgNo === 1 ? 2 : (pgNo) === Math.ceil(totalResults/pgSize) ? Math.ceil(totalResults/pgSize) - 1 : pgNo } 
						</button>

						<button type="button" className="btn btn-light btn-outline-danger" 
							disabled={(totalResults === 0) || (pgNo + 1 > Math.ceil(totalResults/pgSize)) } 
							onClick={() => { 
								setPgNo(totalResults === 0 ? 3 : (pgNo + 1) === 2 ? 3 : (pgNo + 1) === Math.ceil(totalResults/pgSize) + 1 ? Math.ceil(totalResults/pgSize) + 2  : pgNo + 1);
							}
						}>
							{totalResults === 0 ? 3 : (pgNo + 1) === 2 ? 3 : (pgNo + 1) === Math.ceil(totalResults/pgSize) + 1 ? Math.ceil(totalResults/pgSize) : pgNo + 1} 
						</button>
...
function GetCourier()
{
	...
	const tokenId = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJMb2dpbklERW5jcnlwdCI6IkdJb2RoSmpZblVKTkN4MmMwVWFZeGc9PSIsIm5iZiI6MTcwOTk4MDYxNywiZXhwIjoxNzA5OTgxMjE3LCJpYXQiOjE3MDk5ODA2MTd9.uOtFp4kE393gFgQ9o7vtWjVmPNbvYVIIQIcQTi3cZPI';
	...
	const handleSorting = (fildName) => {
	    ...
	    const sortVal = fildName + ' ' + newSortType;
	    console.log("======= handleSorting sortVal " + sortVal);
	    setSortVal(sortVal);
	    GetCourierList(pgSize, pgNo, srchval, sortVal);
	}
	...
	const GetCourierList = (pgSize,pgNo,srchval='',sortVal) => {
	...
	const courierActiveStatus = (courierID,curStatus) => {
		...
		<>
			<div>
				...
				<div className="d-flex justify-content-between"  style={{ marginTop: '5rem', }}>
					...
					<div style={{ marginLeft: '1rem', }} >
						<div className="container-fluid d-flex">
					      	<input type="text" id="crrSearch" name="crrSearch" onChange={(e) => { e.preventDefault(); setSrchval(e.target.value); }} placeholder="Search" className="form-control me-2" />
					      	<button className="fa-solid fa-magnifying-glass btn btn-outline-success" type="button" 
					      		onClick= {() => { GetCourierList(pgSize, pgNo, srchval);}} >
					      	</button>
						</div>
					</div>
				</div>
...
*/}
		        ...
		        <div className="d-flex justify-content-between"  style={{ marginTop: '1rem', }}>
					...
					<div>
						{totalResults === 0 ? 0 : ((srchval !== '' ? 1 : pgNo )- 1) * pgSize + 1} to {Math.min(pgNo * pgSize, totalResults)} out of {totalResults} On Page No. : {srchval !== '' ? 1 : pgNo}
					</div>
